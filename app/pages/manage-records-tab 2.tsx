...
export default function ManageRecordsTab({ token }: { token: string }) {
  ...
  const handlePresentationSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const buffer = shouldValidateBuffer ? Number.parseInt(presentationBuffer, 10) : null;
    if (!presentationTitle.trim() || !Number.isFinite(minutes) || minutes < 1) {
      showMessage("Enter a title and duration.", "error");
      return;
    }
    if (shouldValidateBuffer && (!Number.isFinite(buffer) || buffer < 0)) {
