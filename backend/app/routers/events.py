...
def add_class(
    ...
    try:
        ...
        prof_resp = write.insert("professors", professors_payload) if professors_payload else None
...
def add_professor(
    ...
    try:
        ...
        resp = write.insert("professors", [professor.model_dump()])
        professors_inserted = _rows_affected(resp, fallback=1)
        records_inserted = {"professors": professors_inserted}
        return {
            "status": "inserted",
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to add professor: {exc}"
        ) from exc
...
def add_presentation(
    ...
    try:
        ...
        professor_payload = [
            supabase_schemas.PresentationProfessor(
                id=uuid4(),
                presentation_id=presentation_id,
                professor_id=professor_id,
            ).model_dump()
            for professor_id in (payload.assigned_professors or [])
        ]
        professors_resp = (
            write.insert("presentation_professors", professor_payload)
            if professor_payload
            else None
        )
        ...
        presentation_professors_inserted = _rows_affected(
            professors_resp, fallback=len(professor_payload)
        )

        records_inserted = {
            "presentations": presentations_inserted,
            "presenting_students": presenting_students_inserted,
            "presentation_professors": presentation_professors_inserted,
        }
...
def update_schedule_assignment(
    ...
    try:
        ...
        professor_ids = {pid for profs in professors_by_class.values() for pid in profs}

        def _professor_resources_for(pres: dict[str, Any]) -> set[str]:
            assigned: set[str] = set()
            for prof in pres.get("assigned_professors", []):
                raw_pid = prof.get("id") or prof.get("professor_id")
                ...
                if pid:
                    ...
                    if name:
                        person_name_by_id[pid] = name
            if assigned:
                professor_ids.update(assigned)
                return assigned
            return set(professors_by_class.get(str(pres.get("class_id", "")), []))
        ...
        target_resources: set[str] = _professor_resources_for(target_pres)
        ...
        if other_pres_ids:
            ...
            for other in all_presentations:
                ...
                other_resources: set[str] = _professor_resources_for(other)
                ...
                if shared:
                    ...
                    is_professor = next(iter(shared)) in professor_ids
...
def bulk_update_schedule_assignments(
    ...
    try:
        ...
        def _resources_for(pres: dict[str, Any]) -> set[str]:
            resources: set[str] = _professor_resources_for(pres)
...
def update_presentation(
    ...
    try:
        ...
        updates = payload.model_dump(
            exclude_none=True,
            exclude={"presentation_id", "presenting_students", "assigned_professors"},
        )
        update_payload = _serialize_update_fields(updates)
        presentations_updated = 0
        presenting_students_deleted = 0
        presenting_students_inserted = 0
        presentation_professors_deleted = 0
        presentation_professors_inserted = 0
        ...
        if payload.assigned_professors is not None:
            presentation_professors_delete_resp = (
                supabase.table("presentation_professors")
                .delete()
                .eq("presentation_id", str(payload.presentation_id))
                .execute()
            )
            presentation_professors_deleted = _rows_affected(
                presentation_professors_delete_resp
            )
            presentation_professors_payload = [
                supabase_schemas.PresentationProfessor(
                    id=uuid4(),
                    presentation_id=payload.presentation_id,
                    professor_id=professor_id,
                ).model_dump()
                for professor_id in payload.assigned_professors
            ]
            if presentation_professors_payload:
                presentation_professors_insert_resp = write.insert(
                    "presentation_professors", presentation_professors_payload
                )
                presentation_professors_inserted = _rows_affected(
                    presentation_professors_insert_resp,
                    fallback=len(presentation_professors_payload),
                )

        records_inserted = {"presenting_students": presenting_students_inserted}
        records_inserted["presentation_professors"] = presentation_professors_inserted
        records_deleted = {
            "presenting_students": presenting_students_deleted,
            "presentation_professors": presentation_professors_deleted,
        }
        ...
        return {
            ...
            "assigned_professors_updated": payload.assigned_professors is not None,
            "records_inserted": records_inserted,
            "records_deleted": records_deleted,
            "records_updated": records_updated,
            "lines_edited": _sum_counts(
