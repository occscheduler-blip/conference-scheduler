...
> Generated by [Claude Sonnet 4.6](https://www.anthropic.com/claude) on 2026-03-03 via static analysis of the codebase.
...
### `app/components/backend-status.tsx`

A small utility component embedded in other pages to show backend connectivity.

**State:** `health` (status + environment string), `error`

**On mount:** calls `GET /health` (no auth required). Displays:
- Green status text if backend is reachable
...
│   │   └── schemas.py       # (legacy response models, mostly unused)
│   └── supabase_io/
...
A FastAPI dependency that enforces API key authentication on every protected route.
...
- Returns `401` for wrong key, `500` if the server hasn't configured a key.
- Authentication failures are logged with a partial key hint for debugging.
